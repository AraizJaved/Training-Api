...
namespace Meeting_App.Service
{
    public class FolderService
    {
        ...
        public List<FileDTO> FetchFiles(int id, string searchQuery, Guid userid, int trainingId)
        {
            ...
            using (var db = new IDDbContext())
            {
                ...
                if (id == 0)
                {
                    queryDb = searchQuery?.Length > 1 ? queryDb.Where(o => o.Name.Contains(searchQuery) && o.TrainingId == trainingId) : queryDb.Where(x => x.Fk_Folder == null && x.TrainingId== trainingId);

                }
                else
                {
                    queryDb = searchQuery?.Length > 1 ? queryDb.Where(o => o.Name.Contains(searchQuery) && o.TrainingId == trainingId) : queryDb.Where(x => x.Fk_Folder == id && x.TrainingId == trainingId);
