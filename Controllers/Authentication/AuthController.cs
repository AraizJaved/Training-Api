    ...
    public class AuthController : Controller
    {
        ...
        public async Task<IActionResult> Register([FromBody] UserRegisterDTO model)
        {
            try
            {
                ...
                if (!result.Succeeded)
                    return Ok( new { Status = "Error", Message = "User creation failed! Please check user details and try again." });
                else
                {
                    foreach(string roles in model.Role)
                    {
        ...
        [HttpPost]
        [Route("RegisterParticepants")]
        public async Task<IActionResult> RegisterParticepants([FromBody] ParticepentsRegisterDTO model)
        {
            try
            {
                var ProfileExist = _authService.CheckProfile(model);

                if (ProfileExist)
                {
                    var userExists = await userManager.FindByNameAsync(model.CNIC);
                    //Duplicate Check
                    if (userExists != null)
                        return Ok(new { Status = "Warning", Message = "User Already Exists!" });
                ...
                {
                    return Ok(new { Status = "Failed", Message = "This user is not exist in Hrmis" });
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> DeleteUserById(string Id)
        {
            ...
            try
            {
                var userRole = await userManager.FindByIdAsync(Id);

                var result = await userManager.DeleteAsync(userRole);
